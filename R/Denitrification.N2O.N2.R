#' Denitrification.N2O.N2 abundance information was extracted and combined at different species taxonomic levels
...
##' data(tax)
##' data(abundance)
#' Denitrification.N2O.N2 <- Denitrification.N2O.N2(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group)
#' @author contact: Zhengfu Yue \email{yuezhengfu2011@163.com} Liang Zeng \email{zengliang@biozeron.com}
#' @export

Denitrification.N2O.N2 <- function(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group){
    A <- as.data.frame(Gene$K00376)
    A[A == ""] <- NA
