#' Fer abundance information was extracted and combined at different species taxonomic levels
...
##' data(tax)
##' data(abundance)
#' Fer <- Fer(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group)
#' @author contact: Zhengfu Yue \email{yuezhengfu2011@163.com} Liang Zeng \email{zengliang@biozeron.com}
#' @export


Fer <- function(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group){

    A <- as.data.frame(Gene$K00016)
    A[A == ""] <- NA
    A <- as.data.frame(na.omit(A))
    colnames(A) <- c("V1")
    A <- merge(A,tax,by = "V1")
    K00016 <- merge(A,abundance,by = "V1")
