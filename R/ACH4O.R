#' ACH4O abundance information was extracted and combined at different species taxonomic levels
...
##' data(tax)
##' data(abundance)
#' ACH4O <- ACH4O(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group)
#' @author contact: Zhengfu Yue \email{yuezhengfu2011@163.com} Liang Zeng \email{zengliang@biozeron.com}
#' @export


ACH4O <- function(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group){
    A <- as.data.frame(Gene$K08684)
    A[A == ""] <- NA
    A <- as.data.frame(na.omit(A))
    colnames(A) <- c("V1")
    A <- merge(A,tax,by = "V1")
    K00855 <- merge(A,abundance,by = "V1")
