#' ARS.PAPS.SO3 abundance information was extracted and combined at different species taxonomic levels
...
##' data(tax)
##' data(abundance)
#' ARS.PAPS.SO3 <- ARS.PAPS.SO3(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group)
#' @author contact: Zhengfu Yue \email{yuezhengfu2011@163.com} Liang Zeng \email{zengliang@biozeron.com}
#' @export


ARS.PAPS.SO3 <- function(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group){
    A <- as.data.frame(Gene$K00390)
    A[A == ""] <- NA
