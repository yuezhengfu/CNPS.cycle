#' Nitrification.NH2OH.NO2 abundance information was extracted and combined at different species taxonomic levels
...
##' data(tax)
##' data(abundance)
#' Nitrification.NH2OH.NO2 <- Nitrification.NH2OH.NO2(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group)
#' @author contact: Zhengfu Yue \email{yuezhengfu2011@163.com}
#' @export

Nitrification.NH2OH.NO2 <- function(kegg.category = Gene,NR.tax = tax,profile = abundance,group = group){
    A <- as.data.frame(Gene$K10535)
    A[A == ""] <- NA
