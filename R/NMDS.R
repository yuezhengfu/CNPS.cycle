#' NMDS analysis and visualization
...
nmds.arg <- function(ARG_sub,group){
    colnames(group) <- c("sample","Group")
    ARG_beta <- t(ARG_sub)
    pcoa <- metaMDS(ARG_beta)
    stress <- pcoa$stress
    nmds_scores <- pcoa$points
    nmds_scores <- as.data.frame(nmds_scores)
    nmds_scores$sample <- rownames(nmds_scores)
    nmds_scores <- nmds_scores[,c("sample","MDS1","MDS2")]
    ...
    ARG_pcoa1<-ggplot(plotdata, aes(PC1, PC2)) +
        ...
        labs(title="NMDS - Function genes of carbon cycling") +
        xlab("NMDS1") +
        ylab("NMDS2")+
    ...
    ARG_pcoa2<-ggplot(plotdata, aes(PC1, PC2)) +
        ...
        geom_hline(aes(yintercept = 0),linetype="dotted")+
        scale_fill_manual(values=cbbPalette)+
        labs(title="NMDS - Function genes of cycling") +
        xlab("NMDS1") +
        ylab("NMDS2")+
        theme(text=element_text(size=18))+
        theme(panel.background = element_rect(fill='white', colour='black'),
